...
import { Pencil, ChevronDown, Plus, Eye, Play } from 'lucide-react';
    ...
    <TeacherLayout>
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="flex-1 flex flex-col overflow-y-auto bg-slate-50 relative">
          <main className="p-8 max-w-5xl mx-auto w-full space-y-6 pb-12 flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
              ...
              <div className="space-y-1.5">
                <label className="font-bold text-[11px] text-gray-500 uppercase tracking-widest block">Target Group</label>
                <div className="relative">
                  <select
                    {...methods.register('targetGroup')}
                    className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm appearance-none"
                  >
                    <option value="" disabled>Select target group</option>
                    <option value="Physics 101 - Spring 2024">Physics 101 - Spring 2024</option>
                    <option value="Advanced Math - Fall 2024">Advanced Math - Fall 2024</option>
                  </select>
                  <ChevronDown className="w-4 h-4 text-gray-400 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" />
                </div>
                {errors.targetGroup && <p className="text-red-500 text-xs font-semibold">{errors.targetGroup.message}</p>}
              </div>
            </div>
          ...
          <div className="sticky bottom-0 w-full bg-white border-t border-gray-200 p-4 px-6 md:px-10 flex flex-col sm:flex-row gap-4 justify-between items-center shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)] z-20">
            ...
            <div className="flex items-center gap-3">
              ...
              <button type="submit" className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 rounded-xl text-sm font-bold text-white hover:bg-indigo-700 transition-colors shadow-sm">
                Proceed to Publish
